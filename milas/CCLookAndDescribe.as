/*
	CCLookAndDescribe Multimedia Interactive Learning Application (MILA).
	Copyright © 2013 Matt Bury All rights reserved.
	http://matbury.com/
	matt@matbury.com
*/
package com.matbury.milas {
	...
	public class CCLookAndDescribe extends Sprite {
		
		private var _version:String = "2013.05.21";
		...
		public function CCLookAndDescribe() {
			...
			/* 
			FlashVars.xmlurl = "../commonobjects/swf/common_objects.swf";
			//FlashVars.xmlurl = "../animals/swf/animals.swf";
			initLoadBar();
			positionLoadBar();
			loadData();
			*/
		...
		*/
		...
		private function configureListeners(dispatcher:IEventDispatcher):void {
			dispatcher.addEventListener(Event.INIT, loadedHandler);
			dispatcher.addEventListener(ProgressEvent.PROGRESS, progress);
			dispatcher.addEventListener(IOErrorEvent.IO_ERROR, failedHandler);
		}
		
		private function removeListeners(dispatcher:IEventDispatcher):void {
			dispatcher.removeEventListener(Event.INIT, loadedHandler);
			dispatcher.removeEventListener(ProgressEvent.PROGRESS, progress);
			dispatcher.removeEventListener(IOErrorEvent.IO_ERROR, failedHandler);
		}
		...
		private function failedHandler(event:IOErrorEvent):void {
			showError(Lang.NO_ACTIVITY_DATA);
			positionUserMessage();
		}
		
		private function loadedHandler(event:Event):void {
			removeListeners(event.target.loader.contentLoaderInfo);
			deleteLoadBar();
			var mc:MovieClip = event.target.loader.content as MovieClip;
			_items = mc.items;
		...
		*/
		private function nextImage():void {
			var index:uint = _si.ind[_currentIndex];
			_image = new CCImage(index,_items[index]);
		...
		*/
		...
		private function nextText():void {
			...
			var index:uint = _si.ind[_currentIndex];
			try {
				_sentence = _items[index].answer.text;
			} catch(e:Error) {
				_sentence = "";
			}
		...
		*/
		...
		private function initWrongImages():void {
			...
			for(var i:uint = 0; i < len; i++) {
				var image:CCImage = new CCImage(_wrong[i],_items[_wrong[i]]);
				...
				t.text = _items[_wrong[i]].answer.text;
				t.x = image.x - 40;
				t.y = image.y + 35;
				addChild(t);
				var cross:Cross = new Cross();
		...
		private function imageDown(event:MouseEvent):void {
			var image:CCImage = event.currentTarget as CCImage;
			image.playAnswer();
		}
	}
