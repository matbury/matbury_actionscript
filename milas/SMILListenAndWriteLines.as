/*
    SMILListenAndWriteLines Multimedia Interactive Learning Application (MILA)
...
package com.matbury.milas {
	...
	public class SMILListenAndWriteLines extends Sprite {
		...
		public function SMILListenAndWriteLines() {
		...
		*/
		private function initText():void {
			...
			for(var i:uint = 0; i < lenth; i++) {
				...
				var sntnc:String = _xml.body.seq[i].par.(@id == "answer").text;
				var words:Array = sntnc.split(" ");
		...
		private function positionText():void {
			if(_sentences) {
				...
				var i_len:uint = _sentences.length;
				for(var i:uint = 0; i < i_len; i++) {
					var j_len:uint = _sentences[i].length;
					//play
					_sentences[i][0].x = posX + (_sentences[i][0].width * 0.5);
					_sentences[i][0].y = posY + (_sentences[i][0].height * 0.5);
					posX += _sentences[i][0].width + 3;
					//stretched
					_sentences[i][1].x = posX + (_sentences[i][1].width * 0.5);
					_sentences[i][1].y = posY + (_sentences[i][1].height * 0.5);
					posX += _sentences[i][1].width + 5;
					for(var j:uint = 2; j < j_len; j++) {
						if(posX > stage.stageWidth - (_sentences[i][j].width + 5)) {
							posX = 180;
							posY += _sentences[i][j].height + 5;
						}
						_sentences[i][j].x = posX;
						_sentences[i][j].y = posY;
						posX += _sentences[i][j].width + 3;
					}
					posX = 180;
					posY += _sentences[i][2].height + 5;
				}
			}
		}
